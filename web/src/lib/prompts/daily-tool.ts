// Daily Tool Generation Prompts
// Ported from Android DailyAppGenerationService.kt
...
export interface JournalEntryContext {
  ...
  mood?: string
  tags: string[]
  content: string // HTML stripped, limited to 500 chars
}
...
export function buildDailyToolUserPrompt(
  ...
  previousTools: PreviousToolContext[]
): string {
...
Content: ${entry.content}`
  ...
  if (previousTools.length > 0) {
    const toolsList = previousTools
      .map((tool) => `- "${tool.title}" (${tool.date}): ${tool.description}`)
      .join('\n')
...
export interface GeneratedDailyTool {
  title: string
  description: string
...
/**
 * Parse the Claude response to extract the generated tool
 */
export function parseDailyToolResponse(response: string): GeneratedDailyTool {
  ...
  if (!parsed.title || !parsed.description || !parsed.htmlCode) {
