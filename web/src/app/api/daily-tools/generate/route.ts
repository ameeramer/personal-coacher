...
// Maximum tokens for the generated app (enough for complete HTML/CSS/JS)
const MAX_TOKENS = 8192

// Use Claude Sonnet for tool generation (good balance of quality and speed)
const TOOL_GENERATION_MODEL = 'claude-sonnet-4-20250514'

/**
 * POST /api/daily-tools/generate
 *
 * Generate a new daily tool using Claude API server-side.
 ...
 *
 * Request body:
 * - forceRegenerate?: boolean - If true, regenerate even if one exists for today
 *
 * Returns the generated DailyTool or an error.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if we already have a tool for today (unless forceRegenerate)
    if (!forceRegenerate) {
    ...
    if (recentEntries.length === 0) {
      return NextResponse.json(
        { error: 'No journal entries found in the last 7 days. Please write some journal entries first.' },
        { status: 400 }
      )
    }

    // Get previous tools to avoid duplicates (last 14 days)
    ...
    const userPrompt = buildUserPrompt(recentEntries, previousTools)

    console.log(`[DailyToolGenerate] Generating tool for user ${userId} with ${recentEntries.length} entries, ${previousTools.length} previous tools`)
    ...
    if (!textContent || textContent.type !== 'text') {
      console.error('[DailyToolGenerate] No text content in response')
      return NextResponse.json(
    ...
    const parsedApp = parseGeneratedApp(responseText)
    if (!parsedApp) {
      console.error('[DailyToolGenerate] Failed to parse response:', responseText.substring(0, 500))
      return NextResponse.json(
        { error: 'Failed to parse generated app' },
        { status: 500 }
      )
    }

    // If forceRegenerate, delete existing tool for today first
    if (forceRegenerate) {
      await prisma.dailyTool.deleteMany({
        where: {
          userId,
          date: {
            gte: today,
    ...
    const dailyTool = await prisma.dailyTool.create({
      data: {
        ...
        description: parsedApp.description,
        htmlCode: parsedApp.htmlCode,
        journalContext: parsedApp.journalContext,
        status: 'PENDING'
      }
    })

    console.log(`[DailyToolGenerate] Created tool: ${dailyTool.id} - "${dailyTool.title}"`)

    return NextResponse.json(dailyTool, { status: 201 })
  } catch (error) {
    console.error('[DailyToolGenerate] Error:', error)
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('overloaded') || error.message.includes('529')) {
        return NextResponse.json(
          { error: 'Claude API is overloaded. Please try again in a few minutes.' },
          { status: 503 }
        )
      }
    }
...
function parseGeneratedApp(response: string): ParsedApp | null {
  try {
    // Try to extract JSON from the response
    const jsonString = extractJson(response)
    const json = JSON.parse(jsonString)

    const title = json.title || 'Daily Tool'
    const description = json.description || 'A personalized tool for you'
    const htmlCode = json.htmlCode

    if (!htmlCode || (!htmlCode.includes('<!DOCTYPE html>') && !htmlCode.includes('<html'))) {
      console.error('[DailyToolGenerate] Invalid HTML code in response')
      return null
    }
    ...
    return {
      ...
      description,
      htmlCode,
      journalContext: json.journalContext || null
    }
  } catch (e) {
    console.error('[DailyToolGenerate] Failed to parse JSON:', e)
    return null
  }
}
...
function buildSystemPrompt(): string {
  return `You are a creative personal growth tool designer. Your task is to generate a UNIQUE, interactive web app that helps the user based on their recent journal entries.
...
function buildUserPrompt(entries: JournalEntry[], previousTools: PreviousTool[]): string {
...
Content: ${cleanContent}`
  ...
  if (moods.length > 0) {
    ...
    moodSummary = `Dominant mood: ${dominantMood || 'Mixed'}`
  }
  ...
  if (previousTools.length > 0) {
    const toolsList = previousTools.map(tool => {
      return `- "${tool.title}" (${dateFormatter.format(tool.date)}): ${tool.description}`
    }).join('\n')
...
// Configure Vercel function to allow longer execution time
