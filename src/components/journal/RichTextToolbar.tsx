...
import { isValidUrl } from '@/lib/journal-constants'
...
export function RichTextToolbar({
  ...
  const applyFormat = useCallback((tag: string) => {
    ...
    // For a production app, consider migrating to a proper rich text library
    // like TipTap, Slate, or Lexical. See issue #169 for migration plan.
    switch (tag) {
      case 'bold':
  ...
  const formatButtons = [
    { id: 'bold', icon: 'B', label: 'Bold', shortcut: 'Ctrl+B', className: 'font-bold' },
    { id: 'italic', icon: 'I', label: 'Italic', shortcut: 'Ctrl+I', className: 'italic' },
    { id: 'strikethrough', icon: 'S', label: 'Strikethrough', shortcut: '', className: 'line-through' },
    { id: 'h1', icon: 'H1', label: 'Heading 1', shortcut: '', className: 'font-serif font-semibold text-xs' },
    { id: 'h2', icon: 'H2', label: 'Heading 2', shortcut: '', className: 'font-serif font-semibold text-xs' },
    { id: 'h3', icon: 'H3', label: 'Heading 3', shortcut: '', className: 'font-serif font-semibold text-xs' },
    { id: 'ul', icon: '•', label: 'Bullet List', shortcut: '', className: '' },
    { id: 'ol', icon: '1.', label: 'Numbered List', shortcut: '', className: '' },
    { id: 'quote', icon: '"', label: 'Quote', shortcut: '', className: '' },
    { id: 'link', icon: '🔗', label: 'Link', shortcut: 'Ctrl+K', className: '' },
  ]
  ...
  if (minimal) {
    return (
      <div className="flex items-center gap-0.5 px-3 py-1.5" role="toolbar" aria-label="Text formatting">
        {buttonsToShow.map((format) => (
          <button
            ...
            aria-label={format.shortcut ? `${format.label} (${format.shortcut})` : format.label}
            aria-pressed={activeFormats.has(format.id)}
            className={`
        ...
        <div className="relative color-picker-container">
          ...
          {showColorPicker && (
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 p-3 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-amber-200 dark:border-gray-700 z-20 min-w-[180px]" role="menu" aria-label="Color options">
              ...
              <div className="flex flex-col gap-1">
                {TEXT_COLORS.map((color) => (
                  ...
                  >
                    <span className={`w-5 h-5 rounded-full ${color.colorClass} flex-shrink-0 border-2 ${color.value === '' ? 'border-gray-300 dark:border-gray-500' : 'border-transparent'}`} aria-hidden="true" />
        ...
        {showSourceView && onToggleSourceView && (
          <>
            <div className="w-px h-4 bg-amber-200/50 dark:bg-gray-700/50 mx-1" aria-hidden="true" />
            <button
              type="button"
              onClick={onToggleSourceView}
              title={isSourceView ? "Switch to WYSIWYG view" : "View source code"}
              aria-label={isSourceView ? "Switch to WYSIWYG view" : "View source code"}
  ...
  return (
    <div className="flex flex-wrap gap-1 p-2 bg-amber-50/50 dark:bg-gray-800/50 border-b border-amber-200/50 dark:border-gray-700/50 rounded-t-xl" role="toolbar" aria-label="Text formatting">
      ...
      <div className="relative color-picker-container">
        ...
        {showColorPicker && (
          <div className="absolute top-full left-0 mt-1 p-3 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-amber-200 dark:border-gray-700 z-10 min-w-[180px]" role="menu" aria-label="Color options">
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 text-center">Select text first</p>
            <div className="flex flex-col gap-1">
              {TEXT_COLORS.map((color) => (
                <button
      ...
      <div className="w-px h-6 bg-amber-200 dark:bg-gray-700 mx-1 self-center" aria-hidden="true" />
      ...
      <span className="text-xs text-amber-600/60 dark:text-gray-500 self-center pr-2" aria-live="polite">
